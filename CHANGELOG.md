...
- Added extended Unicode compatibility normalization with:
    ...
    - `OpenCC.normalizeCompatExtended(...)` for combined Unicode compatibility and CJK Compatibility Ideograph
      normalization.
- Added `Unicode_Compatibility.txt` as the built-in extended Unicode compatibility mapping table.
- Added CLI `convert` option `-E` / `--norm-compat-extended` for extended compatibility normalization before conversion.
- Added canonical custom-dictionary slot completion and supported-slot display to CLI `-D` / `--custom-dict` help.
...
- Added a GitHub Actions workflow that builds and tests the Java 8-compatible `openccjava` library on Ubuntu with
  `JDK  21`.
